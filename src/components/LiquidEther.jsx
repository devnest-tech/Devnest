  ...
  useEffect(() => {
    ...
    class MouseClass {
      constructor() {
        ...
        this.takeoverStartTime=0;this.takeoverDuration=0.25;this.takeoverFrom=new THREE.Vector2();
        this.takeoverTo=new THREE.Vector2();this.onInteract=null;
      ...
      init(container) {
        ...
        if(!defaultView)return;this.listenerTarget=defaultView;
    ...
    class AutoDriver {
      ...
      update(){
        ...
        dir.normalize();let ramp=1;
        if(this.rampDurationMs>0){const t=Math.min(1,(now-this.activationTime)/this.rampDurationMs);ramp=t*t*(3-2*t);}
        const step=this.speed*dtSec*ramp;const move=Math.min(step,dist);this.current.addScaledVector(dir,move);
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(props){
        ...
        const csX=props.cursor_size*props.cellScale.x;const csY=props.cursor_size*props.cellScale.y;
        ...
        const u=this.mouse.material.uniforms;u.force.value.set(forceX,forceY);u.center.value.set(cX,cY);u.scale.value.set(props.cursor_size,props.cursor_size);super.update();
      }
    }
    ...
    const io=new IntersectionObserver(entries=>{
      const entry=entries[0];const isVisible=entry.isIntersecting&&entry.intersectionRatio>0;
      isVisibleRef.current=isVisible;if(!webglRef.current)return;
    ...
    const ro=new ResizeObserver(()=>{
      if(!webglRef.current)return;if(resizeRafRef.current)cancelAnimationFrame(resizeRafRef.current);
