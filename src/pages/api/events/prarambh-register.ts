...
export default async function handler(
  ...
  try {
    ...
    if (!fullName || !email || !phone || !rollNumber || !college || !branch || !year || !competition || !teamName) {
      return res.status(400).json({
        error: "Missing required fields. Please fill in all required details including Team Name and Roll Number.",
      });
    }
